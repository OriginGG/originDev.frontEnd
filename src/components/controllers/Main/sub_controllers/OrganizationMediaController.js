...
class OrganizationMediaController extends Component {
	...
	componentDidMount = async () => {
		...
		const OrganizationMediaComponentRender = loadable(
			(props) =>
				import(/* webpackChunkName: "renderComponents" */ `../../../render_components/themes/${props.theme}/OrganizationMediaComponentRender`),
			{
		...
		const OrganizationTwitchComponentRender = loadable(
			(props) =>
				import(/* webpackChunkName: "renderComponents" */ `../../../render_components/themes/${props.theme}/OrganizationTwitchComponentRender`),
			{
				fallback: <div>Loading...</div>
			}
		);
		const OrganizationYouTubeComponentRender = loadable(
			(props) =>
				import(/* webpackChunkName: "renderComponents" */ `../../../render_components/themes/${props.theme}/OrganizationYouTubeComponentRender`),
			{
				fallback: <div>Loading...</div>
			}
		);
		...
		this.upcoming_enigma_style = {
			borderColor: this.props.uiStore.current_organisation.primaryColor,
			backgroundColor: 'transparent'
		};
		this.recent_enigma_style = {
			borderColor: this.props.uiStore.current_organisation.primaryColor,
			backgroundColor: this.props.uiStore.current_organisation.primaryColor
		};
		...
		const youTubeChannels = await this.props.appManager.executeQuery('query', getYouTubeChannelsQuery, {
		...
		if (this.props.uiStore.current_organisation.streamTeamUrl) {
			team_l = await axios.get(
				`${process.env.REACT_APP_API_SERVER}/twitch/get-team-member?teamurl=${this.props.uiStore
					.current_organisation.streamTeamUrl}`
			);
			// console.log(`NEW TEAM CALL ${JSON.stringify(team_l)}`);
		} else {
			console.log('no team name');
		}
		const users = await this.props.appManager.executeQuery('query', getRosterQuery, {
			organisationId: this.props.uiStore.current_organisation.id,
		...
		this.setState({
			...
			roster_list: t_array,
			visible: true,
			OrganizationMediaComponentRender,
			OrganizationTwitchComponentRender,
			OrganizationYouTubeComponentRender
		});
	...
	handleYoutubeClick = () => {
		...
		const u_e_style = (this.recent_enigma_style = {
			borderColor: this.props.uiStore.current_organisation.primaryColor,
			backgroundColor: this.props.uiStore.current_organisation.primaryColor
		});
		const r_e_style = (this.recent_enigma_style = {
			borderColor: this.props.uiStore.current_organisation.primaryColor,
			backgroundColor: 'transparent'
		});
	...
	render() {
		...
		y_array.push(<OrganizationYouTubeComponentRender theme={theme} video_url={this.state.video1_url} />);
		y_array.push(<OrganizationYouTubeComponentRender theme={theme} video_url={this.state.video2_url} />);
		y_array.push(<OrganizationYouTubeComponentRender theme={theme} video_url={this.state.video3_url} />);
		y_array.push(<OrganizationYouTubeComponentRender theme={theme} video_url={this.state.video4_url} />);
			...
			if (this.state.team_list.data.success) {
				this.state.team_list.data.data.users.forEach((l, i) => {
					if (l.live) {
						...
						const t_url = `https://player.twitch.tv/?channel=${l.name}`;
						const t_thumb =
							'https://res.cloudinary.com/origingg/image/upload/f_auto/v1551819112/game_images/twitch_offline.png';
						p_array.push(
							<OrganizationTwitchComponentRender
								theme={theme}
								key={`twitch_live_k_${i}`}
