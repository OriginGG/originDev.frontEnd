...
class OrganizationBlogListController extends Component {
	...
	componentDidMount = async () => {
        ...
        const OrganizationNewsComponentRender = loadable(
            (props) =>
                import(/* webpackChunkName: "renderComponents" */ `../../../render_components/themes/${props.uiStore
                    .current_theme_full_name}/OrganizationNewsComponentRender`),
            {
                fallback: <div>Loading...</div>
            }
        );
		...
		if (this.isMobile()) {
			if (theme === 'felzec/light' || theme === 'enigma2/dark') {
                OrganizationNewsMobileComponentRender = loadable(
                    (props) =>
                        import(/* webpackChunkName: "renderComponents" */ `../../../render_components/themes/${props.uiStore
                            .current_theme_full_name}/OrganizationNewsMobileComponentRender`),
                    {
                        fallback: <div>Loading...</div>
                    }
                );
		...
		blog_data.resultData.edges.forEach((blog, i) => {
			...
			if (this.isMobile() && theme === 'felzec/light') {
				this.blog_array.push(
                    <OrganizationNewsMobileComponentRender
                        uiStore={this.props.uiStore}
						key={`news_blog_item_k_${i}`}
						blog={blog}
				...
				this.blog_array.push(
                    <OrganizationNewsComponentRender
                        uiStore={this.props.uiStore}
						key={`news_blog_item_k_${i}`}
						blog={blog}
						blog_date={formattedDate}
	...
	render() {
		...
		const theme = `${this.props.uiStore.current_organisation.themeBaseId}/${this.props.uiStore.current_organisation
			.themeId}`;
				...
				<BlogModal
					...
					content={
                        <OrganizationNewsModalComponentRender
							...
							blog_media={this.state.blog_media}
