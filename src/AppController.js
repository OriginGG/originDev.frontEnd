	...
	componentDidMount = async () => {
		...
		if (is_root) {
			...
			if (
				...
				(location.pathname === '/go_paywall' && this.ignore_routes === false)
			) {
				// eslint-disable-line
				const domainInfo = this.props.appManager.getDomainInfo();
				const subDomain =
				...
				if (!domainInfo) {
					...
					if (o.resultData === null) {
						...
						if (auth) {
							const my_org = await this.props.appManager.executeQuery('query', getOrganisationByIdQuery, {
								id: auth.authenticate.resultData.organisationId
							});
